...
def ls_tree_sizes(ref: str, include: List[str], exclude: List[str]) -> Dict[str, int]:
    ...
    if code != 0:
        raise RuntimeError(f"git ls-tree failed for {ref!r}: {err.strip()}")
    entries = out.split('\x00')
    sizes: Dict[str, int] = {}
    for e in entries:
        if not e:
            continue
        # format: <mode> <type> <object>\t<size>\t<path>  (with --long)
        # Example piece (not literally): "100644 blob abcd123\t1234\tpath/to/file"
        # But with -z, fields may still be separated; safer approach:
        try:
            # We can split once at tab to get the size and then path
            before, after = e.split('\t', 1)  # before contains "MODE TYPE SHA"
            size_str, path = after.split('\t', 1)
            size = int(size_str.strip())
            path = path.strip()
        except Exception:
            # Fallback robust parse: take last tab as separator between size and path
            if '\t' in e:
                parts = e.split('\t')
...
def main() -> int:
    try:
        ...
        if code != 0:
            # Try fetching remote if it's a remote ref
            if "/" in ref_base:
